...
private Routes routes = new Routes();
	...
	public SharekhanConnect(String vendorKey,String apiKey,String accessToken) {
	...
	public String getLoginURL(String apiKey,String vendor_key,String version_id,String state) throws NullPointerException {
		
		String baseUrl = routes.getLoginUrl() + "api_key="+ apiKey ;

		if (vendor_key != null) {
			baseUrl += "&vendor_key=" + vendor_key;
			} else {
	...
	public JSONObject generateSession( String apiKey,String request_Token,String vendor_key, String state,String secretKey,Long version_id) {
		try {
			...
			String decData=decodeToken(secretKey,token);
			String[] result = decData.split("\\|");
//		    for(String s : result){System.out.println(s);} 
		    String manipulatedcode = result[1] +"|"+ result[0];
		    String encData= encodeToken(secretKey,manipulatedcode);
			...
			params.put("vendorKey", vendor_key);
...
//		byte[] original = null;
	...
	public JSONObject generateSessionWithoutVersionId( String apiKey,String request_Token,String vendor_key, String state,String secretKey) {
		try {
...
//		    System.out.println(requestToken);
	...
	public JSONObject placeOrder(OrderParams orderParams) {
		try {
			...
			JSONObject jsonObject = null;
				...
				jsonObject = sharekhanAPIRequestHandler.postRequest(this.vendorKey,this.apiKey,url, params, this.accessToken);
				return jsonObject;
	...
	public JSONObject modifyorder(OrderParams orderParams) {
		try {
			...
			JSONObject jsonObject = sharekhanAPIRequestHandler.postRequest(this.vendorKey,this.apiKey, url, params, this.accessToken);
			return jsonObject;
		} catch (Exception | SharekhanAPIException e) {
			System.out.println(e.getMessage());
			return null;
...
//	API retrieves all orders for the day.
	public JSONObject getOrder(Long customerId) {
		try {
			...
			System.out.println(url);
			JSONObject response = sharekhanAPIRequestHandler.getRequest(this.vendorKey,this.apiKey, url,this.accessToken);
